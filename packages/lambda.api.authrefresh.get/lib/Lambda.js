...
class ApiAuthRefreshPostLambda extends LambdaApiBase {
    constructor({ config }) {
    ...
    static Create(...args) {
        return new ApiAuthRefreshPostLambda(...args);
    }
    ...
    async impl(event) { // eslint-disable-line class-methods-use-this
        const func = `${ApiAuthRefreshPostLambda.name}.impl`;
        event.logger.info({ function: func, log: 'started' });

        // const interfaces = getDBInterfaces(this.services);
        // const { empty } = event.validated;

        const body = '<html><head></head><body>Hello World: refresh</body></html>';
    ...
    async buildResponse({ statusCode, hateoasResponse }, { logger }) { // eslint-disable-line class-methods-use-this
        const func = `${ApiAuthRefreshPostLambda.name}.buildResponse`;
...
module.exports = ApiAuthRefreshPostLambda;
